...
atakbotz.ev.on('group-participants.update', async (data) => {
	try {
            let metadata = await atakbotz.groupMetadata(data.id)
	  for (let i of data.participants) {
		try {
		  var pp_user = await atakbotz.profilePictureUrl(i, 'image')
		} catch {
		  var pp_user = 'https://i0.wp.com/www.gambarunik.id/wp-content/uploads/2019/06/Top-Gambar-Foto-Profil-Kosong-Lucu-Tergokil-.jpg'
		}
		if (data.action == "add") {
		  atakbotz.sendMessage(data.id, { image: { url: pp_user }, caption: `Hallo @${i.split("@")[0]}\nSelamat Datang Di Grup ${metadata.subject}\n\nmenu jasa promosi ketik ( /jsmenu )\n\n\nmenu store ketik ( /menu ) : \nT E S T I : https://t.me/puttstore_id :\nOWNER : wa.me/+6285695710527 :\n\nSemoga Kamu Senang Berada Disini Serta Jangan Lupa Untuk Membaca Dan Mematuhi Rules Yang Ada`, mentions: [i] })
		} else if (data.action == "remove") {
		  atakbotz.sendMessage(data.id, { image: { url: pp_user }, caption: `Goodbye @${i.split("@")[0]}\n\nTetap Putus Asa Jangan Semangat Dan Jadilah Beban Keluarga 🤙🗿`, mentions: [i] })
		}
	  }
	} catch (e) {
	  console.log(e)
